...
const DOMKeyTable:{[s:string]:number[]} = {};

function addStandard(key:string, standard:number) {
    if (standard === undefined) throw new Error("Undefined keysym for key \"" + key + "\"");
    if (key in DOMKeyTable) throw new Error("Duplicate entry for key \"" + key + "\"");
    DOMKeyTable[key] = [standard, standard, standard, standard];
}

function addLeftRight(key:string, left:number, right:number) {
...
function addNumpad(key:string, standard:number, numpad:number) {
    if (standard === undefined) throw new Error("Undefined keysym for key \"" + key + "\"");
