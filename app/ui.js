...
const UI = {
    ...
    initSettings() {
        ...
        llevels.forEach(llevel =>
            UI.addOption(document.getElementById('noVNC_setting_logging'),llevel, llevel)
        );
...
* ------v------*/
    ...
    addControlbarHandlers() {
        ...
        exps.forEach(exp =>
            exp.addEventListener('click', UI.toggleExpander)
        );
    },
 ...
 * ------v------*/
    ...
    connectFinished() {
        ...
        let msg = UI.getSetting('encrypt')
            ? _("Connected (encrypted) to ") + UI.desktopName
            : _("Connected (unencrypted) to ") + UI.desktopName;
    ...
    securityFailed(e) {
        const msg = ('reason' in e.detail)
            ? _("New connection has been rejected with reason: ") +
                e.detail.reason
            : _("New connection has been rejected");
 ...
 * ------v------*/
    ...
    updateViewClip() {
        ...
        const new_clip = UI.getSetting('view_clip') || isTouchDevice;
...
// Set up translations
const LINGUAS = ["de", "el", "nl", "pl", "sv", "zh"];
l10n.setup(LINGUAS);
